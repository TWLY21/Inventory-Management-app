...
export default function ProductDetailScreen({navigation, route}) {
  ...
  async function handleSave() {
    try {
      ...
      Alert.alert(
        'Success',
        isCreateMode ? 'Product created.' : 'Product updated.',
      );
      navigation.goBack();
    } catch (error) {
      Alert.alert(
        'Unable to save',
        error.response?.data?.message || 'Please review the fields.',
      );
    } finally {
      setSubmitting(false);
    }
    ...
    <ScreenContainer>
      <SectionCard
        title={
          isCreateMode
            ? 'Create Product'
            : existingProduct?.name || 'Product Detail'
        }
        subtitle={
          isAdmin
        ...
        {isAdmin ? (
          <>
            <PrimaryButton
              title={isCreateMode ? 'Create Product' : 'Save Changes'}
              onPress={handleSave}
              loading={submitting}
            />
            {!isCreateMode ? (
